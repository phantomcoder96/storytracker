...
## Project Overview
Automated Instagram Story Notifications is a highly resilient, automated script designed to track targeted Instagram accounts, extract their stories (including images and embedded links), and deliver them directly to an email inbox.

## Motivation
The primary motivation for this project was to establish a healthier relationship with social media while maintaining access to critical career opportunities. Mindless scrolling on Instagram is a massive time-sink, but several niche tech accounts exclusively post highly valuable job applications, networking events, and startup news via their Instagram Stories. 

This system allows me to completely remove Instagram from my phone while ensuring I never miss a high-priority opportunity. The data is parsed and sent straight to my inbox, securely and automatically.
...
- **Resilient Systemd Automation (Arch Linux)**: The script is scheduled locally via native Arch Linux `systemd` timers with `Persistent=true`. If the computer is asleep during a scheduled run, the Linux kernel automatically triggers a catch-up execution the exact second the machine reconnects to the network.
